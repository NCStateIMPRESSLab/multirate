...
from model_multirate_convmmnet import multirate_base_layer_convmmnet, multirate_enhancement_layer_convmmnet;
...
# %% Defining the model
def measurement_model_convmmnet(original_model, im_size, mes_max, channel):
    input_image = tf.keras.Input(shape=(im_size, im_size, channel));
    flatten = tf.keras.layers.Flatten()(input_image);
    measurement = tf.keras.layers.Dense(mes_max)(flatten);
    measurement_model = tf.keras.Model(inputs=input_image,
                                       outputs=measurement);
    
    for i in range(1,3):
        measurement_model.layers[i].set_weights(original_model.layers[i].get_weights())
    
    return measurement_model;

def reconstruction_model_convmmnet(original_model, im_size, mes_step, channel):
    # Reconstruction layer
    input_layer = tf.keras.Input(shape=(im_size**2,));
    base = multirate_base_layer_convmmnet(input_layer[:,:mes_step], im_size, mes_step, channel);
    en1 = multirate_enhancement_layer_convmmnet(base, input_layer[:,mes_step:2*mes_step], im_size, mes_step, channel);
    en2 = multirate_enhancement_layer_convmmnet(en1, input_layer[:,2*mes_step:3*mes_step], im_size, mes_step, channel);
    en3 = multirate_enhancement_layer_convmmnet(en2, input_layer[:,3*mes_step:4*mes_step], im_size, mes_step, channel);
    en4 = multirate_enhancement_layer_convmmnet(en3, input_layer[:,4*mes_step:5*mes_step], im_size, mes_step, channel);
    en5 = multirate_enhancement_layer_convmmnet(en4, input_layer[:,5*mes_step:6*mes_step], im_size, mes_step, channel);
    en6 = multirate_enhancement_layer_convmmnet(en5, input_layer[:,6*mes_step:7*mes_step], im_size, mes_step, channel);
    en7 = multirate_enhancement_layer_convmmnet(en6, input_layer[:,7*mes_step:8*mes_step], im_size, mes_step, channel);
    en8 = multirate_enhancement_layer_convmmnet(en7, input_layer[:,8*mes_step:9*mes_step], im_size, mes_step, channel);
    en9 = multirate_enhancement_layer_convmmnet(en8, input_layer[:,9*mes_step:10*mes_step], im_size, mes_step, channel);
    en10 = multirate_enhancement_layer_convmmnet(en9, input_layer[:,10*mes_step:11*mes_step], im_size, mes_step, channel);
    en11 = multirate_enhancement_layer_convmmnet(en10, input_layer[:,11*mes_step:12*mes_step], im_size, mes_step, channel);
    en12 = multirate_enhancement_layer_convmmnet(en11, input_layer[:,12*mes_step:13*mes_step], im_size, mes_step, channel);
    en13 = multirate_enhancement_layer_convmmnet(en12, input_layer[:,13*mes_step:14*mes_step], im_size, mes_step, channel);
    en14 = multirate_enhancement_layer_convmmnet(en13, input_layer[:,14*mes_step:15*mes_step], im_size, mes_step, channel);
    en15 = multirate_enhancement_layer_convmmnet(en14, input_layer[:,15*mes_step:16*mes_step], im_size, mes_step, channel);
    
    reconstruction_model = tf.keras.Model(inputs=input_layer,
                                     outputs=[base, en1, en2, en3, en4, en5, en6, en7,#]);
                                              en8, en9, en10, en11, en12, en13, en14, en15]);
    
    for i in range(1,len(reconstruction_model.layers)):
        reconstruction_model.layers[i].set_weights(original_model.layers[i+2].get_weights());
...
MEASUREMENT_NUM = int(1024 * MEASUREMENT_RATE);
...
MODEL_ADDRESS = "./models/multirate_convmmnet_"+str(BLOCK_SIZE)+"/model.keras"; 

# %% Importing the trained model
print("Importing the trained model"); 
model = tf.keras.models.load_model(MODEL_ADDRESS);

measurement_model = measurement_model_convmmnet(model, IMAGE_SIZE, IMAGE_SIZE**2, NUM_CHANNEL);
reconstruction_model = reconstruction_model_convmmnet(model, IMAGE_SIZE, BLOCK_SIZE, NUM_CHANNEL);
...
for root, dirs, files in os.walk(TEST_IMAGE_ADDRESS, topdown=True):
    for name in files:
       ...
       measurements = np.concatenate([measurements[:,:MEASUREMENT_NUM], np.zeros(shape=(measurements.shape[0],nB - MEASUREMENT_NUM))], axis=1);
...
print("/nAverage PSNR value of "+TEST_DATA+" dataset: ", avg_psnr);
print("Average SSIM value of "+TEST_DATA+" dataset: ", avg_ssim);
print("Average LPIPS value of "+TEST_DATA+" dataset: /n", avg_lpips);
...
cv2.imshow("Predicted Image (Multirate)", cv2_predicted_image); 
cv2.waitKey(0); 
#cv2.imwrite(TEST_DATA+"_predicted_image_"+str(index)+"_multirate_"+str(int(100*MEASUREMENT_RATE))+".png", predicted_images[index]);
...
cv2.waitKey(0); 
cv2.imwrite(TEST_DATA+"_predicted_image_patch_"+str(index)+"_multirate_"+str(int(100*MEASUREMENT_RATE))+".png", patch); 